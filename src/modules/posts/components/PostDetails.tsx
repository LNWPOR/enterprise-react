...
const CommentList = lazy(() =>
  import('modules/comments/components/CommentList')
);

const PostList: FC = () => {
  const dispatch = useDispatch();
  const { id } = useParams<RouteParams>();
  const { isLoading } = useSelector(selectors.getPosts);
  const product = useSelector(selectors.getPost(id));
  ...
  return (
    <>
      ...
      <div ref={ref}>{inView && <CommentList postId={id}></CommentList>}</div>
    </>
  );
};
